...
export class S3Utils {
     ...
     */
    public async copyFile(fileURL: string, newBucket: string, newKey: string): Promise<string> {
        try {
    ...
    public async renameFile(fileURL: string, newName: any) {
        try {
            let s3Elements = S3Utils.urlDeconstruct(fileURL);
            let copyResult = await this.copyFile(fileURL, s3Elements.bucket, `${s3Elements.path}${newName}`);
            if (copyResult) {
